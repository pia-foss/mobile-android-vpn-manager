...
internal class GetProtocolConfigurationTest {

    @Test
    fun `should succeed retrieving from cache the set protocol configuration information`() = runTest {
        // given
        ...
        val getProtocolConfiguration: IGetProtocolConfiguration =
            GetProtocolConfiguration(cacheProtocol = cacheProtocol)

        // when
        val setResult = setProtocolConfiguration(protocolConfiguration = protocolConfigurationMock)
        val getResult = getProtocolConfiguration()

        // then
        assert(setResult.isSuccess)
        assert(getResult.isSuccess)
        assert(getResult.getOrThrow() == protocolConfigurationMock)
    }

    @Test
    fun `should fail retrieving from cache when there is no protocol configuration information`() = runTest {
        // given
        val context: Context = ApplicationProvider.getApplicationContext()
        val cacheProtocol: ICacheProtocol = GivenExternal.cache(context = context)
        val getProtocolConfiguration: IGetProtocolConfiguration =
        ...
        // when
        val result = getProtocolConfiguration()
