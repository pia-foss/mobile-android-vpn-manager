...
internal class GetTargetProtocol(
    private val cacheProtocol: ICacheProtocol,
) : IGetTargetProtocol {

    // region IGetTargetProtocol
    override suspend fun invoke(): Result<VPNProtocolTarget> {
        val protocolConfiguration = cacheProtocol.getProtocolConfiguration().getOrElse {
            return Result.failure(it)
        }

        return Result.success(protocolConfiguration.protocolTarget)
    }
    // endregion
}
