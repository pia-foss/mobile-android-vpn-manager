...
internal class Serializer : ISerializer {
    ...
    // region ISerializer
    override fun deserialize(value: String): Result<WireguardAddKeyResponse> =
        try {
            Result.success(json.decodeFromString(value))
        } catch (exception: SerializationException) {
            Result.failure(
                VPNProtocolError(code = VPNProtocolErrorCode.DESERIALIZATION_FAILURE)
            )
        }
    // endregion
}
