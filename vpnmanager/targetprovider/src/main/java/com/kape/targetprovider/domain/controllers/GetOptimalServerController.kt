...
internal class GetOptimalServerController(
    private val setServerList: ISetServerList,
    private val getOptimalServer: IGetOptimalServer,
) : IGetOptimalServerController {

    // region IGetOptimalServerController
    override suspend fun invoke(serverList: TargetProviderServerList): Result<TargetProviderServer> =
        setServerList(serverList = serverList)
            .mapCatching {
                getOptimalServer().getOrThrow()
            }
    // endregion
}
