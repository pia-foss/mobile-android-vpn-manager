...
internal class SetWireguardTunnelHandleTest {

    @Test
    fun `should update cache with the set wireguard tunnel handle response`() = runTest {
        // given
        ...
        // when
        val result = setWireguardTunnelHandle(tunnelHandle = tunnelHandleMock)

        // then
        assert(result.isSuccess)
        assert(cacheWireguard.getWireguardTunnelHandle().getOrThrow() == tunnelHandleMock)
    }
}
