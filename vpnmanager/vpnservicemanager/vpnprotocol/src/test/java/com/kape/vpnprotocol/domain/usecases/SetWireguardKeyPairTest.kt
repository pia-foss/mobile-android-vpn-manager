...
internal class SetWireguardKeyPairTest {

    @Test
    fun `should update cache with the set wireguard key pair response`() = runTest {
        // given
        val context: Context = ApplicationProvider.getApplicationContext()
        val wireguardKeyPairMock: IWireguardKeyPair = WireguardKeyPairMock()
        val cacheWireguardKeys: ICacheWireguardKeys = GivenExternal.cache(context = context)
        val setWireguardKeyPair: ISetWireguardKeyPair =
            SetWireguardKeyPair(cacheKeys = cacheWireguardKeys)

        // when
        val result = setWireguardKeyPair(wireguardKeyPair = wireguardKeyPairMock)

        // then
        assert(result.isSuccess)
        assert(cacheWireguardKeys.getKeyPair().getOrThrow() == wireguardKeyPairMock)
    }
}
