...
internal class SetServiceFileDescriptorTest {

    @Test
    fun `should update cache with the set service file descriptor information`() = runTest {
        // given
        ...
        val setServiceFileDescriptor: ISetServiceFileDescriptor =
            SetServiceFileDescriptor(cacheService = cacheService)

        // when
        val result = setServiceFileDescriptor(serviceConfigurationFileDescriptorProviderMock)

        // then
        assert(result.isSuccess)
        assert(cacheService.getServiceFileDescriptorProvider().getOrThrow() == serviceConfigurationFileDescriptorProviderMock)
    }
}
