...
internal class ConnectionEventCallback(
    private val connectivityStatusChangeCallback: VPNServiceConnectivityStatusChangeCallback,
) : IConnectionEventCallback {

    override fun handleConnectivityStatusChange(status: VPNManagerConnectionStatus) {
        connectivityStatusChangeCallback.handleConnectivityStatusChange(status)
    }

    override fun handleMtuTestResult(localToRemote: Int, remoteToLocal: Int) {
        connectivityStatusChangeCallback.handleMtuTestResult(localToRemote = localToRemote, remoteToLocal = remoteToLocal)
    }
}
